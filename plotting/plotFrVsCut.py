...
def plotFrVsCut(frLeft2, frLeft1, frMed, frRight1, frRight2, massCut, output):
    ...
    g_sigma2.Draw("f")
    g_sigma1.Draw("f")
    ly = ROOT.TLegend(0.7, 0.8, 0.85, 0.9)
    ly.SetTextSize(0.03)
    ly.SetBorderSize(0)
    ...
    g_med.GetXaxis().SetLabelSize(0.035)
    #g_med.GetXaxis().SetLabelOffset(1.05)
    g_med.GetYaxis().SetTitle("flavor ratio")
    g_med.GetYaxis().SetTitleOffset(1.2)
    g_med.GetXaxis().SetTitleOffset(1.2)
    g_med.GetYaxis().SetLabelSize(0.035)
    #g_med.GetYaxis().SetLabelOffset(1.05)
